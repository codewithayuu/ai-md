...
if [ $# -gt 0 ]; then
  case "$1" in
    ...
    "--help"|"-h")
      ...
      echo ""
      echo "  --version VERSION - Set version number (default: manifest.json)"
...
if [ -z "$VERSION" ]; then
  if command -v jq &> /dev/null; then
    VERSION="$(jq -r '.version' "$EXT_DIR/manifest.json" 2>/dev/null || true)"
  else
    VERSION="$(grep -oE '"version"[[:space:]]*:[[:space:]]*"[^"]+"' "$EXT_DIR/manifest.json" 2>/dev/null | head -n 1 | sed -E 's/.*"version"[[:space:]]*:[[:space:]]*"([^"]+)".*/\1/' || true)"
  fi
fi

if [ -z "$VERSION" ]; then
  echo "Error: Could not read version from $EXT_DIR/manifest.json"
  exit 1
fi
...
build_chrome() {
  ...
  rm -f "$DIST_DIR/AI-MD-Chrome-v$VERSION.zip" 2>/dev/null
  ...
  (cd "$CHROME_DIR" && zip -r "$DIST_DIR/AI-MD-Chrome-v$VERSION.zip" * -q)
  
  echo "Chrome package created: $DIST_DIR/AI-MD-Chrome-v$VERSION.zip"
  return 0
}
...
build_firefox() {
  ...
  rm -f "$DIST_DIR/AI-MD-Firefox-v$VERSION.zip" 2>/dev/null
  ...
  (cd "$FIREFOX_DIR" && zip -r "$DIST_DIR/AI-MD-Firefox-v$VERSION.zip" * -q)
  
  echo "Firefox package created: $DIST_DIR/AI-MD-Firefox-v$VERSION.zip"
  return 0
}
...
build_source() {
  ...
  rm -f "$DIST_DIR/AI-MD-Source-v$VERSION.zip" 2>/dev/null
  ...
  (cd "$SOURCE_DIR" && zip -r "$DIST_DIR/AI-MD-Source-v$VERSION.zip" * -q)
  
  echo "Source package created: $DIST_DIR/AI-MD-Source-v$VERSION.zip"
  return 0
}
